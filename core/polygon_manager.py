...
class PolygonManager:
    def __init__(self, filepath="configs/polygons.json"):
            ...
            if os.path.exists(self.filepath):
                try:
                    with open(self.filepath, 'r') as f:
                        ...
                        logger.info(f"Loaded polygons from {os.path.abspath(self.filepath)}")
                except Exception as e:
                    logger.error(f"Failed to load polygons from {os.path.abspath(self.filepath)}: {e}")
                    self.polygons = {}
            else:
                logger.warning(f"Polygons file not found at {os.path.abspath(self.filepath)}")
                self.polygons = {}
